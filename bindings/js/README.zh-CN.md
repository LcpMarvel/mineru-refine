...
const { items, report } = await refine(contentList, {
  ...
  modelConfig,                 // 可选:配置驱动换模型(见下文「换模型」)
});
...
**1. `modelConfig`——配置驱动、多厂商（推荐）。** 一个含两个独立角色的对象(`reasoning`
文本、`vision` 视觉),每个是 `{ provider?, model, key?, baseUrl? }`。省略某角色则回落
env 默认。
...
**2. 自定义回调——逃生口（优先级高于 `modelConfig`）。** 回调是
`refine(items, opts?, onProgress?, chat?, visionJudge?, visionTranscribe?)` 的第 4~6 个位置参数:

```ts
const chat = async (messages, tools) =>
  // messages: OpenAI 风格数组, tools: 数组
...
const visionTranscribe = async (img: Buffer, cellsRender: string) =>  // 可选;仅 rewriteGarbledTables 用
