...
| `modelConfig` | 无 | 配置驱动换模型:把文本(`reasoning`)和/或视觉(`vision`)角色指向任意多厂商 / OpenAI 兼容 LLM(DeepSeek、Qwen、OpenAI、Anthropic、Gemini、MiniMax、私有化端点……)。省略的角色回落 env 默认(DeepSeek/Qwen)。详见下文[换模型](#换模型自定义-llm) |
...
默认文本角色是 DeepSeek、视觉角色是 Qwen-VL——但不锁死。文本(`reasoning`)与视觉
(`vision`)两个角色可各自独立指向不同的 LLM。有两条机制,按优先级排列。

### 1. `modelConfig`——配置驱动、多厂商（推荐）

基于 [genai](https://github.com/jeremychone/rust-genai) crate,原生认识 DeepSeek、
Aliyun(Qwen/DashScope)、OpenAI、Anthropic、Gemini、Ollama、Groq、xAI,以及**任意
OpenAI 兼容的自定义端点**。传一个含两个独立角色的 `modelConfig`,省略某角色则回落 env 默认。

每个角色是 `{ provider?, model, key?, baseUrl? }`:

| 字段 | 含义 |
|---|---|
| `provider` | 厂商/协议:`deepseek` / `aliyun`(`qwen`、`dashscope`) / `openai`(亦 `openai-compatible`、`custom`) / `anthropic`(`claude`) / `gemini`(`google`) / `ollama` / `groq` / `xai`(`grok`)。省略则从模型名推断 |
| `model` | 模型名(如 `deepseek-chat`、`qwen-vl-max`、`gpt-4o`、`MiniMax-M3`) |
| `key` | API key。省略则回落该厂商的默认环境变量 |
| `baseUrl` | OpenAI 兼容端点(私有化部署 / 自定义网关)。省略则用该厂商默认端点 |

**示例——MiniMax-M3**（OpenAI 兼容且原生多模态,一个模型同时充当两个角色）:
...
要求:文本端点须支持 **tool-call**(`tool_choice: "required"`),视觉端点须接受图片输入。
会吐 `<think>…</think>` 推理块的模型(MiniMax、DeepSeek-R1、QwQ……)已妥善处理——
推理块被自动剥离,绝不污染裁决产物。`modelConfig` 进缓存 key(按模型身份),换模型绝不
会错误命中另一个模型的缓存。裁决质量随模型而异;保真闸与 fail-open 仍兜底(改坏回滚,
最差原样返回),换新模型时建议先拿几份真实文档对比 `report`。

### 2. 自定义回调——逃生口

当 `modelConfig` 表达不了你需要的鉴权/代理/模型逻辑时,注入你自己的 `chat` / `vision`
实现。它们优先级高于 `modelConfig`,`modelConfig` 又高于 env 默认。各语言的回调形状见
分绑定文档:[Python](bindings/python/)(对象/可调用)、[JS](bindings/js/)(async 回调)、
